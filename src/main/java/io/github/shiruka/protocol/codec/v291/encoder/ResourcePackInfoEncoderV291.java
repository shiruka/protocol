 ...
 */
@PacketId(6)
public final class ResourcePackInfoEncoderV291
  extends PacketEncoder.Base<ResourcePackInfo> {

  @Override
  public void decode(
    @NotNull final ResourcePackInfo packet,
    @NotNull final CodecHelper helper,
    @NotNull final PacketBuffer buffer,
    @NotNull final MinecraftSession session
  ) {
    packet.forcedToAccept(buffer.readBoolean());
    packet.behaviorPackInfos(
      buffer.readArrayShortLE(() -> helper.readResourcePackInfoEntry(buffer))
    );
    packet.resourcePackInfos(
      buffer.readArrayShortLE(() -> helper.readResourcePackInfoEntry(buffer))
    );
  }
  ...
  public void encode(
    ...
    buffer.writeArrayShortLE(
      packet.behaviorPackInfos(),
      entry -> helper.writeResourcePackEntry(buffer, entry)
    );
    buffer.writeArrayShortLE(
      packet.resourcePackInfos(),
      entry -> helper.writeResourcePackEntry(buffer, entry)
